...
def _normalize_paypal_currency(raw_currency: Optional[str]) -> str:
    # Accept only ISO currencies supported by the current PayPal flow.
    if not raw_currency:
...
def _normalize_paypal_amount(raw_amount: Optional[str]) -> str:
    # Ensure the amount is a numeric string compatible with PayPal.
    if raw_amount is None or str(raw_amount).strip() == "":
        raise HTTPException(status_code=400, detail="amount is required")
    try:
        amount_value = Decimal(str(raw_amount)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
...
def _get_paypal_credentials(db: Session) -> Tuple[str, str, str, str]:
    # Prefer environment variables in production, fallback to stored admin config when present.
    ...
    mode = (env_mode or (config.mode if config else "sandbox")).lower()
    currency = env_currency or (config.currency_code if config else "USD")
    return client_id, client_secret, mode, currency
...
def paypal_create_order(payload: PayPalOrderRequest, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    client_id, client_secret, mode, _ = _get_paypal_credentials(db)
    if not client_id or not client_secret:
        raise HTTPException(status_code=400, detail="PayPal not configured")

    currency_code = _normalize_paypal_currency(payload.currency)
    amount_value = _normalize_paypal_amount(payload.amount)

    base_url = "https://api-m.sandbox.paypal.com" if mode == "sandbox" else "https://api-m.paypal.com"
    access_token = _get_paypal_access_token(client_id, client_secret, base_url)

    # Build a minimal PayPal order payload using ISO currency codes only.
    order_payload = {
        "intent": "CAPTURE",
        "purchase_units": [
            {
    ...
    logger.info("PayPal create-order: challenge_id=%s currency=%s amount=%s", payload.challenge_id, currency_code, amount_value)
