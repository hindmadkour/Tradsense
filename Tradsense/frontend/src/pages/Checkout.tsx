...
const loadPayPalScript = (clientId: string, currencyCode: string) =>
  new Promise<void>((resolve, reject) => {
    ...
    const encodedCurrency = encodeURIComponent(currencyCode);
    script.src = `https://www.paypal.com/sdk/js?client-id=${encodedClientId}&currency=${encodedCurrency}&intent=capture&components=buttons`;
    script.async = true;
...
const Checkout = () => {
  ...
  const paypalCurrency = (paypalConfig?.currency_code || envPayPalCurrency || 'USD').toUpperCase();
  const paypalAmount = selectedChallenge ? Number(selectedChallenge.price_dh || 0).toFixed(2) : null;
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (window.paypal && container) {
          window.paypal.Buttons({
            createOrder: async () => {
              if (!paypalAmount) {
                throw new Error('Missing PayPal amount');
              }
    ...
    setupPayPal();
  }, [paypalConfig, selectedChallenge, paypalAmount, paypalCurrency, toast, navigate]);
